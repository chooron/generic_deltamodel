...
infil_vic_arno = lambda r, soil, param_dict: r * (1 - (1 - soil / param_dict['vadose_max_level'])
                                                  ** param_dict['vic_beta'])
infil_hbv = lambda r, soil, param_dict: r * ((1 - soil / param_dict['vadose_max_level']) ** param_dict['hbv_beta'])

# quickflow equations group
quickflow_linear = lambda soil, param_dict: param_dict['quickflow_k'] * soil
quickflow_vic = lambda soil, param_dict: param_dict['mmax'] * (soil / param_dict['vadose_max_level']) \
                                         ** param_dict['quickflow_n']
quickflow_topmodel = lambda soil, param_dict: param_dict['mmax'] * (param_dict['vadose_max_level'] / param_dict['n']) \
                                              * 1 / (param_dict['lamb'] ** param_dict['quickflow_n']) \
                                              * (soil / param_dict['vadose_max_level']) ** param_dict['quickflow_n']

# baseflow equations group
baseflow_linear = lambda soil, param_dict: (10.0 ** param_dict['baseflow_k']) * soil
baseflow_nolinear = lambda soil, param_dict: (10.0 ** param_dict['baseflow_k']) * soil ** param_dict['baseflow_n']
...
class blendv2(torch.nn.Module):
    ...
    def unpack_parameters(self, parameters: torch.Tensor, weights_group: list) -> tuple[torch.Tensor, None]:
        ...
        for weights in weights_group:
            ...
            one_hot_result = torch.zeros_like(phy_params)
            one_hot_result.scatter_(dim=2, index=tmp_max_indices, value=1.0)
            phy_params[:, :, mask, :] = one_hot_result
    ...
    def forward(
        ...
        # 初始化模型状态
        SNOW_ON_GROUND = torch.zeros([3, n_grid, self.nmul], dtype=torch.float32, device=self.device) + self.nearzero
        WATER_IN_SNOWPACK = torch.zeros([3, n_grid, self.nmul], dtype=torch.float32, device=self.device) + self.nearzero
        CUMSNOWMELT = torch.zeros([3, n_grid, self.nmul], dtype=torch.float32, device=self.device) + self.nearzero
    ...
    def PBM(
        ...
        for t in range(n_steps):
            ...
            infil_2 = infil_vic_arno(water_available, VADOSE_LEVEL, param_dict)
            infil_3 = infil_hbv(water_available, VADOSE_LEVEL, param_dict)
            infiltration = torch.clamp(param_dict['w4'] * infil_1 + param_dict['w5'] * infil_2 \
                                       + param_dict['w6'] * infil_3, min=self.nearzero, max=water_available)

            horizontal_transfert_mu[t, :, :, 0] = water_available - infiltration
            VADOSE_LEVEL = VADOSE_LEVEL + infiltration - RET
            # -- 快速流计算 --
            quickflow_1 = quickflow_linear(VADOSE_LEVEL, param_dict)
            quickflow_2 = quickflow_vic(VADOSE_LEVEL, param_dict)
            quickflow_3 = quickflow_topmodel(VADOSE_LEVEL, param_dict)
            horizontal_transfert_mu[t, :, :, 1] = torch.clamp(param_dict['w7'] * quickflow_1 \
                                                              + param_dict['w8'] * quickflow_2 \
                                                              + param_dict['w9'] * quickflow_3,
                                                              min=self.nearzero, max=VADOSE_LEVEL)
            # -- 基流计算 --
            baseflow_1 = baseflow_linear(VADOSE_LEVEL, param_dict)
            baseflow_2 = baseflow_nolinear(VADOSE_LEVEL, param_dict)
            ...
            PHREATIC_LEVEL = PHREATIC_LEVEL + vadose2phreatic
